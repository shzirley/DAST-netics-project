...
\newpage
\section*{Lampiran: Antarmuka Aplikasi Web (Customer)}
\addcontentsline{toc}{section}{Lampiran: Antarmuka Aplikasi Web}

Berikut adalah dokumentasi visual dari antarmuka \textit{website} CineTrack bagian pelanggan (\textit{Customer Front-End}) yang disusun dalam bentuk \textit{storyboard}.

\begin{center}
\begin{longtable}{|c|p{6cm}|p{7cm}|}
\caption{\textit{Storyboard} Keseluruhan Website CineTrack} \label{tab:storyboard} \\
\hline
\textbf{No.} & \multicolumn{1}{c|}{\textbf{Deskripsi}} & \multicolumn{1}{c|}{\textbf{Gambar}} \\
\hline
\endfirsthead

\multicolumn{3}{c}%
{{\bfseries Lanjutan Tabel \ref{tab:storyboard}}} \\
\hline
\textbf{No.} & \multicolumn{1}{c|}{\textbf{Deskripsi}} & \multicolumn{1}{c|}{\textbf{Gambar}} \\
\hline
\endhead
...
1. & Pada \textit{screen} 1 menampilkan Beranda Pelanggan (Dashboard) yang memuat daftar film yang sedang tayang beserta kategorinya secara dinamis dari \textit{View} \texttt{vw\_film\_genre}. & \vspace{0.2cm}\includegraphics[width=6.5cm]{website/home_user.jpeg}\vspace{0.2cm} \\
\hline
2. & Pada \textit{screen} 2 menampilkan halaman Penjelajahan Film (Movies Catalog) untuk mencari detail film spesifik menggunakan fungsi \textit{search} langsung dari tabel \texttt{film}. & \vspace{0.2cm}\includegraphics[width=6.5cm]{website/movies.jpeg}\vspace{0.2cm} \\
\hline
3. & Pada \textit{screen} 3 menampilkan halaman Pemilihan Jadwal (Showtimes) untuk memilih waktu dan studio. Data ditarik otomatis dari \textit{View} \texttt{vw\_detail\_jadwal}. & \vspace{0.2cm}\includegraphics[width=6.5cm]{website/showtimes_user.jpeg}\vspace{0.2cm} \\
\hline
4. & Pada \textit{screen} 4 menampilkan halaman Pemilihan Kursi (Seat Selection) yang memetakan ketersediaan kursi secara \textit{real-time} melalui eksekusi \textit{Function} \texttt{CekStatusKursi()}. & \vspace{0.2cm}\includegraphics[width=6.5cm]{website/seat_user.jpeg}\vspace{0.2cm} \\
\hline
5. & Pada \textit{screen} 5 menampilkan halaman Pemesanan Makanan (Kantin) untuk menambahkan produk makanan ringan dan minuman. & \vspace{0.2cm}\includegraphics[width=6.5cm]{website/kantin_user.jpeg}\vspace{0.2cm} \\
\hline
6. & Pada \textit{screen} 6 menampilkan halaman Pembayaran (Order Summary). Tombol final ini memicu \textit{Stored Procedure} dan \textit{Triggers} untuk menyimpan data ke \texttt{transaksi} dan \texttt{tiket}. & \vspace{0.2cm}\includegraphics[width=6.5cm]{website/payment_user.jpeg}\vspace{0.2cm} \\
\hline
7. & Pada \textit{screen} 7 menampilkan Manajemen Tiket (My Tickets) yang menyajikan riwayat tiket aktif dan \textit{Loyalty Points} yang diambil dari fungsi \texttt{GetTotalTiketByPelanggan}. & \vspace{0.2cm}\includegraphics[width=6.5cm]{website/tickets_user.jpeg}\vspace{0.2cm} \\
\hline
8. & Pada \textit{screen} 8 menampilkan halaman \textit{Executive Dashboard} (Admin) yang menyajikan data \textit{real-time} pendapatan studio dan pelanggan VIP. Metrik ini dihasilkan dari eksekusi \textit{Functions}. & \vspace{0.2cm}\includegraphics[width=6.5cm]{website/dashboard_employee.jpeg}\vspace{0.2cm} \\
\hline
9. & Pada \textit{screen} 9 menampilkan halaman Point of Sale (POS) \textit{Bookings} yang bertindak sebagai kasir untuk operasional internal bioskop. & \vspace{0.2cm}\includegraphics[width=6.5cm]{website/pos_bookings.jpeg}\vspace{0.2cm} \\
\hline
10. & Pada \textit{screen} 10 menampilkan halaman \textit{Manage Movies} yang berfungsi untuk melakukan operasi CRUD (tambah, edit, hapus) langsung ke dalam tabel \texttt{film}. & \vspace{0.2cm}\includegraphics[width=6.5cm]{website/manages_movie.jpeg}\vspace{0.2cm} \\
\hline
11. & Pada \textit{screen} 11 menampilkan halaman \textit{Manage Schedules} yang berfungsi untuk memetakan penugasan tabel \texttt{film} dan \texttt{studio} ke tabel persimpangan \texttt{jadwal\_tayang}. & \vspace{0.2cm}\includegraphics[width=6.5cm]{website/manages_schedule.jpeg}\vspace{0.2cm} \\
\hline
12. & Pada \textit{screen} 12 menampilkan halaman \textit{Manage F\&B} yang terhubung dengan operasi mutasi stok makanan dan minuman dengan pengawasan \textit{Triggers} pada \textit{database}. & \vspace{0.2cm}\includegraphics[width=6.5cm]{website/manages_fnb.jpeg}\vspace{0.2cm} \\
\hline
13. & Pada \textit{screen} 13 menampilkan halaman \textit{Manage Branches} untuk pengaturan data teater/cabang yang merepresentasikan data pada tabel \texttt{cabang} dan \texttt{studio}. & \vspace{0.2cm}\includegraphics[width=6.5cm]{website/manage_branches.jpeg}\vspace{0.2cm} \\
\hline
14. & Pada \textit{screen} 14 menampilkan halaman \textit{HR \& User Management} yang mencakup daftar kepegawaian serta tabel \textit{Idle Customers} yang bersumber dari \textit{View} \texttt{vw\_pelanggan\_idle}. & \vspace{0.2cm}\includegraphics[width=6.5cm]{website/hrstaff.jpeg}\vspace{0.2cm} \\
\end{longtable}
\end{center}
